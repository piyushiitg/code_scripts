...
class MssqlAutoFailover(object):
    ...
    def process_failover(self, cid, old_master, new_master, servers_info, root_account_info, wait_for_sync, \
                         max_wait_sync_retry, wait_sync_retry_interval, force_failover, mirror_role_check_time, log):
        ...
        if self.topology == 'SQLMirror':
            self.failover_obj = SqlMirrorFailover(cid, old_master, new_master, servers_info,\
                                          ...
                                          max_wait_sync_retry, wait_sync_retry_interval, force_failover, mirror_role_check_time)
...
class SqlMirrorFailover(object):
    ...
    def __init__(self, cid, old_master, new_master, servers, root_account_info, log, topology,\
                       wait_for_sync, max_wait_sync_retry, wait_sync_retry_interval, force_failover, mirror_role_check_time):
       self.cluster_id = cid
       ...
       self.unsuccessful_dbids = []
       self.sync_witness_dbids = []
       self.async_dbids_list_on_primary = []
       ...
       self.mirror_role_check_time = mirror_role_check_time
       self.send_alert_msg = ""
            ...
            if self.principal_server_health:
                if mirror_info["mirroring_role"] == 1: # 1 means PRINCIPAL for Async/Sync on that server
                    if mirror_info['mirroring_safety_level'] == 2: # For Sync
                        ...
                        self.async_dbids_list_on_primary.append(database_id)
    ...
    def revert_replication_if_any(self):
        ...
        '''
        self.log.info("Not reverting any replication changes currently because we could not revert Alter commands")
        return
        ...
        if self.principal_server_health:
            ...
            for dbid in self.selected_dbids:
                ...
                if self.execute_query_with_retry(cursor, query2):
                    ...
                    is_revert = True
                    self.unsuccessful_dbids.append(dbid)
                    self.log.info("Failed to Execute Alter command for database_id %s" %dbid)
            ...
            error_msg = ""
            if is_revert == True:
                self.log.error("Failed to execute Alter command for database ids %s, but could not revert Alter command"\
                               " on successful database ids %s" %(self.unsuccessful_dbids, self.successful_dbids))
                error_msg = "SqlServer Failover: Failed to execute Alter command for database ids %s" %self.unsuccessful_dbids
                #self.revert_replication_changes_for_primary() 
                #return False, error_msg
            if len(self.successful_dbids) == 0:
                self.log.error("No Replication changes has been done for cluster id %s" %self.cluster_id)
                return False, error_msg
            ...
            if len(self.successful_dbids) > 0 or len(self.sync_witness_dbids) > 0:
                ...
                if len(self.unsuccessful_dbids) > 0:
                    error_msg = "SqlServer Failover: Failed to execute Alter command for database ids %s" %self.unsuccessful_dbids
                return True, error_msg 
    ...
    def find_recovery_rate(self, db_name):
        ...
        self.log.info("Query on msdb for finding recovery_rate is %s" %query2)
        recovery_rate = None
        try:
            connection = self.get_secondary_server_connection()
            if not connection:
    ...
    def find_mirroring_state(self, db_name, from_primary):
        query1 = "use msdb;"
        query2 = "EXEC sp_dbmmonitorresults %s, 0, 1" %db_name
        self.log.info("Query on msdb for finding mirroring_state is %s" %query2)
        mirroring_state = None
        try:
            if from_primary:
                connection = self.get_primary_server_connection()
            else:
    ...
    def _get_mirroring_state_of_servers(self, from_primary=True):
        self.log.debug("Find Out mirroring state of all database ids")
        mirroring_state_dict = {}
        for dbid in self.selected_dbids:
            mirroring_state = None
            db_name = self.database_info[dbid]["name"]
            mirroring_state = self.find_mirroring_state(db_name, from_primary)
            self.log.info("Response from sp_dbmmonitorresults for mirroring state %s on database name %s" %(mirroring_state, db_name))
            mirroring_state_dict[dbid] = mirroring_state
        return mirroring_state_dict

    def get_dbnames_suspended_mode(self, state_dict):
        '''
        get database names whose mode is suspended mode 
        '''
        db_list = []
        for dbname, mirroring_state in state_dict.iteritems():
            if int(mirroring_state) == 0:
                db_list.append(dbname)
        return db_list

    def resume_database_from_suspended(self, dbnames, from_primary=True):
        ''' Execute query to change suspended mode to resume mode
        '''
        if from_primary:
            connection = self.get_primary_server_connection()
        else:
        ...
        for db_name in dbnames:
            query2 = "ALTER DATABASE %s SET PARTNER RESUME" %db_name
            self.log.info("Executing query %s for dbid %s" %(query2, db_name))
            if self.execute_query_with_retry(cursor, query2):
                self.log.info("Successfully executed Resume command on db_name %s" %db_name)
            else:
                self.log.info("Failed to Execute Resume command for db_name %s" %db_name)
        if cursor:
            cursor.close()
    ...
    def check_for_mirror_role_after_replication(self):
        ...
        endTime = datetime.datetime.now() + datetime.timedelta(minutes=self.mirror_role_check_time)
        ...
        if all_server_become_principal:
            ...
            if self.principal_server_health:
                #self.revert_replication_changes_for_primary()
                self.log.error("All the database Server could not become Principal Server")
                return True
        return True
    ...
    def do_failover(self):
       try:
           ...
           if self.principal_server_health:
               self.log.info("Connecting with current primary server %s" %self.old_master_ip)
               self.connection_old_master = self.get_primary_server_connection()
           
           # Make a connection with secondary for wait for sync
           self.log.info("Connecting with current secondary Server %s" %self.new_master_ip)
           ...
           if self.principal_server_health:
               if not self.connection_old_master:
                   success = False
                   abort_failover = True
                   error_msg = "SqlServer: Aborting failover because could not connect to the current primary server %s"\
                               "on clusterid %s" %(self.old_master_ip, self.cluster_id)
                   return success, abort_failover, error_msg

           if not self.connection_new_master:
               success = False
               abort_failover = True
               error_msg = "SqlServer: Aborting failover because could not connect to the current secondary server %s"\
                           "on clusterid %s" %(self.new_master_ip, self.cluster_id)
               return success, abort_failover, error_msg
           ...
           if len(self.database_info) == 0:
               if self.principal_server_health:
                   self.log.info("There are no PRINCIPAL database configured on current primary server on clusterid %s."\
                                 " But we are doing failover." % self.cluster_id)
               else:
                   self.log.info("There are no MIRROR database configured on current secondary server on clusterid %s."\
                                 " But we are doing failover." % self.cluster_id)
               success = True
               error_msg = ""
               abort_failover = False
           ...
           if len(self.selected_dbids) == 0:
               if self.principal_server_health:
                   self.log.info("Principal server is UP and could not find database ids for replication changes,"\
                                 " so aborting the failover") 
                   error_msg = "SqlServer Failover Abort: Principal server health is UP but could not find database ids"\
                               " for replication changes"
                   abort_failover = True
               else:
                   if len(self.sync_witness_dbids) == 0:
                       error_msg = "SqlServer Failover Abort: Principal server Health is Down and could not find database ids"\
                                   " for replication changes"
                       self.log.info("Principal server Health is Down and could not find database ids for replication changes,"\
                                     " so aborting the failover") 
                       abort_failover = True
           elif len(self.selected_dbids) != len(self.database_info):
               if self.principal_server_health:
                   if len(self.async_dbids_list_on_primary) > 0:
                       error_msg = "SqlServer Failover Abort: Principal server health is UP but some of the database servers"\
                                   " is configured as Async"
                       self.log.info("Principal server is UP but some of the database server is configured as Async, so aborting the failover")
                   else:
                       error_msg = "SqlServer Failover Abort: Principal server health is UP but some of the database servers"\
                                   " are not in the same state either sync/async."
           ...
           # Check Mirroring State Suspended or Resume
           if self.principal_server_health:
               dbnames = []
               primary_mirroring_state = self._get_mirroring_state_of_servers(from_primary=True)
               dbnames = self.get_dbnames_suspended_mode(primary_mirroring_state)
               check_with_secondary = True
               if dbnames:
                   self.log.info("List of database names where Replication in Suspended mode is %s" %dbnames)
                   self.resume_database_from_suspended(dbnames, from_primary=True)
                   #Checking the Mirroring State
                   primary_mirroring_state = self._get_mirroring_state_of_servers(from_primary=True)
                   new_dbnames = self.get_dbnames_suspended_mode(primary_mirroring_state)
                   if new_dbnames:
                       self.log.error("There are still some databases whose replication is in Suspended mode %s" %new_dbnames)
                   else:
                       self.log.info("There are no database whose replication in Suspended mode")
                       check_with_secondary = False
                   if check_with_secondary:
                       # Checking on Secondary server
                       secondary_mirroring_state = self._get_mirroring_state_of_servers(from_primary=False)
                       dbnames = self.get_dbnames_suspended_mode(secondary_mirroring_state)
                       if dbnames:
                           self.log.info("List of database names where Replication in Suspended mode is %s" %dbnames)
                           self.resume_database_from_suspended(dbnames, from_primary=True)
                           #Checking the Mirroring State
           ...
           success = self.check_for_mirror_role_after_replication()

           if self.send_alert_msg:
               return True, False, self.send_alert_msg
           else:
               return True, False, ""
